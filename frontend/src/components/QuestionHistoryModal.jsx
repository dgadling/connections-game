        ...
        {arr(history).length===0 ? <div className="text-subtle">No edits yet.</div> : (
          <ul className="space-y-2 max-h-64 overflow-auto">
            {arr(history).slice().reverse().map((h, i, rev) => {
              ...
              return <li key={h.id} className="border-b border-faint pb-2 text-xs">
                <div className="text-subtle">{h.edited_at ? new Date(h.edited_at).toLocaleString() : ''} · {h.edited_by_name || h.edited_by}</div>
                <div className="text-secondary">
                  {changedText ? <span><span className="text-faint">&quot;{h.old_text}&quot;</span><span className="mx-1">→</span><span>&quot;{newer.text}&quot;</span></span> : null}
                  {!changedText && changedTag ? <span><span className="text-faint">tag {h.old_tag}</span><span className="mx-1">→</span><span>{newer.tag}</span></span> : null}
                  {!changedText && !changedTag ? <span className="text-faint">[{h.old_tag}] {h.old_text}</span> : null}
                </div>
              </li>
            })}
          </ul>
