export function QuestionToolbar({
  ...
  return (
    <div className="bg-surface rounded-xl shadow-sm border border-default p-3 sm:p-4 text-foreground">
      ...
      {status === 'upcoming' && !archived && (
        <>
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            <button type="button" onClick={seedQuestions} disabled={busy} className="px-3 py-1.5 border border-strong rounded-lg hover:bg-surface-muted disabled:opacity-50 text-foreground">📦 Load starter pack</button>
            <button type="button" onClick={()=>setShowImport(v=>!v)} disabled={busy} className="px-3 py-1.5 border border-strong rounded-lg hover:bg-surface-muted disabled:opacity-50 text-foreground">📥 Import</button>
            <button type="button" onClick={doExport} disabled={busy} className="px-3 py-1.5 border border-strong rounded-lg hover:bg-surface-muted disabled:opacity-50 text-foreground">📤 Export</button>
            {qsLength > 1 && <button type="button" onClick={shuffleQuestions} disabled={busy} className="px-3 py-1.5 border border-strong rounded-lg hover:bg-surface-muted ml-auto disabled:opacity-50 text-foreground">🔀 Shuffle</button>}
          </div>
          {Boolean(showImport) && (
            <div className="mt-3 p-3 bg-surface-muted rounded-lg border border-default text-foreground">
              ...
              <textarea value={importText} onChange={e=>setImportText(e.target.value)}
                ...
                className="w-full border border-strong rounded-lg px-3 py-2 text-sm h-32 focus:outline-none focus:ring-2 focus:ring-primary disabled:bg-surface-hover text-foreground bg-surface" />
              <div className="flex gap-2 mt-2">
                ...
                <button type="button" onClick={()=>{setShowImport(false); setImportText('')}} disabled={busy} className="px-3 py-1.5 border border-strong rounded-lg text-xs hover:bg-surface disabled:opacity-50 text-foreground">Cancel</button>
      ...
      {status !== 'upcoming' && (
        <div className="flex gap-2 text-xs">
          <button type="button" onClick={doExport} disabled={busy} className="px-3 py-1.5 border border-strong rounded-lg hover:bg-surface-muted disabled:opacity-50 text-foreground">📤 Export {status}</button>
        </div>
      )}
    </div>
  )
