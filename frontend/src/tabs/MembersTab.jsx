...
{!archived && <div className="bg-surface rounded-xl shadow-sm border border-default p-4 sm:p-5">
      ...
      <div className="bg-surface rounded-xl shadow-sm border border-default divide-y divide-default">
        {loading ? <div className="p-4 text-subtle text-sm">Loading…</div> : null}
        {!loading && active.map(m => (
          <div key={m.id} className="p-3 sm:p-4">
            {editing === m.id ? (
              <div className="flex flex-col sm:flex-row gap-2">
                <input value={editName} onChange={e=>setEditName(e.target.value)} required disabled={saving} className="flex-1 border border-strong rounded-lg px-3 py-2 text-sm disabled:bg-surface-muted" />
                <input value={editDiscord} onChange={e=>setEditDiscord(e.target.value)} placeholder="Discord username (optional)" disabled={saving} className="flex-1 border border-strong rounded-lg px-3 py-2 text-sm disabled:bg-surface-muted" />
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={()=>setEditing(null)} disabled={saving} className="flex-1 sm:flex-initial px-3 py-2 border border-strong rounded-lg text-sm disabled:opacity-60">Cancel</button>
                </div>
              </div>
            ) : (
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                {!archived && <div className="flex gap-4 text-xs text-subtle">
                  <button type="button" onClick={()=>{setEditing(m.id); setEditName(m.name); setEditDiscord(m.discord_id||'')}} className="hover:text-foreground py-1">Edit</button>
                  <button type="button" onClick={()=>delMember(m)} className="hover:text-danger py-1">Delete</button>
                </div>}
              </div>
            )}
          </div>
      ...
      {deleted.length > 0 && (
        <div className="bg-surface rounded-xl shadow-sm border border-default p-4">
          ...
          {Boolean(showDeleted) && (
            <ul className="mt-2 space-y-1 text-sm text-subtle">
              {deleted.map(m => <li key={m.id} className="flex justify-between py-1.5 border-t border-faint"><span>{m.name}</span><button type="button" onClick={()=>restore(m)} className="text-xs hover:text-secondary">Restore</button></li>)}
            </ul>
          )}
        </div>
      )}
